...
cv::Mat AVM_seg_left_gray = cv::Mat::zeros(400,400, CV_8UC1);
cv::Mat AVM_seg_right_gray = cv::Mat::zeros(400,400, CV_8UC1);

cv::Mat aggregated_img    = cv::Mat::zeros(400, 400 , CV_8UC3);
cv::Mat aggregated_seg_img = cv::Mat::zeros(400, 400 , CV_8UC3);
cv::Mat aggregated_seg_img_gray = cv::Mat::zeros(400, 400 , CV_8UC1);
...
void CallbackPhantom_temp(const ocamcalib_undistort::VisionPhantomnetDataList::ConstPtr &msg) {
    ...
    // cout << msg->nets_data[1].camera_id << endl;
    cv::Mat cv_frame_resize_pad = cv_bridge::toCvCopy( msg->nets_data[0].image_viz, msg->nets_data[0].image_viz.encoding )->image;
...
//
    ...
    sub_tmp = nodeHandle.subscribe("/phantomvision/phantomnets", 1, CallbackPhantom_temp);
